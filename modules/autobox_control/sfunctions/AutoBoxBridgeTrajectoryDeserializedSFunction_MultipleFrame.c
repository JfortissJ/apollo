...
#define S_FUNCTION_NAME AutoBoxBridgeTrajectoryDeserializedSFunction_MultipleFrame /* Defines and Includes */
...
#include "client/bridge_receiver_autobox_trajectory.h"
#include "client/bridge_receiver_preprocessing.h"
#include "_init_sfunc_outputs_trajectory.h"
...
static void mdlInitializeSizes(SimStruct* S)
{
	...
	initialize_outputs_trajectory(S);
...
static void mdlUpdate(SimStruct* S, int_T tid)
{
	...
	if(*input_status)
	{
		...
		if(*size > 0)
		{
			is_ready_diser = handle_message_Check(proto_buf, buf, &proto_buf->has_init_, *size);
		}
	}
}
...
static void mdlOutputs(SimStruct* S, int_T tid)
{
	...
	{
		Apollo__Planning__ADCTrajectoryToAutoboxBridge* apollo_msg = handle_message_planning(proto_buf);
		if(apollo_msg != NULL)
		{
			fill_outputs_trajectory(S, apollo_msg);
		}
		else
		{
	...
	{
		FORTISS_INFO_PRINTF("\n-------has maximum total frame but error-----!Traj!----------\n");
		proto_buf->has_init_ = false;
		proto_buf->is_ready_diser = false;
        proto_buf->has_max_total_frames_ = false;
		free(proto_buf->status_list_);
