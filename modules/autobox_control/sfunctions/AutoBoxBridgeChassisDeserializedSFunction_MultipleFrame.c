...
#define S_FUNCTION_NAME AutoBoxBridgeChassisDeserializedSFunction_MultipleFrame
#define S_FUNCTION_LEVEL 2

#define DSPACE_UDP_RECEIVE_SIZE_UINT8 256
...
#include "client/bridge_receiver_autobox_chassis.h"
#include "client/bridge_receiver_preprocessing.h"
#include "modules/autobox_bridge/proto/autobox_chassis.pb-c.h"
#include "_init_sfunc_outputs_chassis.h"
...
static void mdlInitializeSizes(SimStruct* S)
{
	...
	initialize_outputsPort_CanBus_Chassis(S);
...
static void mdlOutputs(SimStruct* S, int_T tid)
{
	...
	{
		Apollo__Canbus__ChassisToAutoboxBridge* apollo_msg = handle_message_chassis(proto_buf);
		if(apollo_msg != NULL)
		{
			fill_outputsPort_CanBus_Chassis(S, apollo_msg);
		}
		else
		{
	...
	{
		FORTISS_INFO_PRINTF("\n-------has maximum total frame but error----!Chassis!-----------\n");
		proto_buf->has_init_ = false;
		proto_buf->is_ready_diser = false;
        proto_buf->has_max_total_frames_ = false;
		free(proto_buf->status_list_);
