...
#define S_FUNCTION_NAME AutoBoxBridgeLocalizationDeserializedSFunction_MultipleFrame /* Defines and Includes */
#define S_FUNCTION_LEVEL 2

#define NO_MESSAGE_PROCESSED (-1)
...
#define MAX_MESSAGE_SIZE_BYTES 50000
...
#include "modules/autobox_bridge/proto/autobox_localization.pb-c.h"
#include "client/bridge_receiver_autobox_localization.h"
#include "client/bridge_receiver_preprocessing.h"
#include "_init_sfunc_outputs_localization.h"
...
/*
 * Specify the number of inputs, outputs, states, parameters, and
 * other characteristics of the C MEX S-function
 */
static void mdlInitializeSizes(SimStruct* S)
{
	int iPort;  // Index of an input port.
	...
	// Specify the number of input ports that this S-function has
	...
	initialize_outputs_localization(S);
...
static void mdlOutputs(SimStruct* S, int_T tid)
{
	...
	{
		Apollo__Localization__LocalizationToAutoboxBridge* apollo_msg = handle_message_localization(proto_buf);
		if(apollo_msg != NULL)
		{
			fill_outputs_localization(S, apollo_msg);
		}
		else
		{
	...
	{
		FORTISS_INFO_PRINTF("\n-------has maximum total frame but error----!Loca!-----------\n");
		proto_buf->has_init_ = false;
		proto_buf->is_ready_diser = false;
        proto_buf->has_max_total_frames_ = false;
		free(proto_buf->status_list_);
